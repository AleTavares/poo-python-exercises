...
class DisplayTemperatura(Observer):
    def update(self, temperatura, umidade, pressao):
        print(f"[Display Temperatura] Temperatura: {temperatura}°C")


class DisplayUmidade(Observer):
    def update(self, temperatura, umidade, pressao):
        print(f"[Display Umidade] Umidade: {umidade}%")


class DisplayCompleto(Observer):
    def update(self, temperatura, umidade, pressao):
        print(f"[Display Completo] Temp: {temperatura}°C | Umidade: {umidade}% | Pressão: {pressao} hPa")
...
if __name__ == "__main__":
    ...
    estacao.definir_medicoes(25, 60, 1013)
    estacao.definir_medicoes(30, 55, 1008)
